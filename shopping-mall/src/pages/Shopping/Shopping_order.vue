      ...
      <div class="checkout-steps">
        ...
        <div class="step-cont">
          <div class="form-new clearfix">
            <div v-for="(value,index) in addressInfo" :key="index">
              <div v-if="value.is_default == 0">
                <div class="add_mi" style="background: url(/static/img/mail.jpg) no-repeat; background-size: 100% 100% !important;" @click.stop="selDefault(value.address_id)">
                  <p style="border-bottom:1px dashed #ccc;">{{value.address_info|typeFilter(2)}}<br />({{value.address_info|typeFilter(1)}} 收)</p>
                  <p>{{value.address_info|typeFilter(3)}}<br />{{value.address_info|typeFilter(4)}}</p>
                  <p @click.stop="isEdit(index,true)" class="edit">编辑</p>
                </div>
              </div>
              <div v-else>
                <div class="add_mi" style="background: url(/static/img/mail_1.jpg) no-repeat; background-size: 100% 100% !important;">
                  <p style="border-bottom:1px dashed #ccc;">{{value.address_info|typeFilter(2)}}<br />({{value.address_info|typeFilter(1)}} 收)</p>
                  <p>{{value.address_info|typeFilter(3)}}<br />{{value.address_info|typeFilter(4)}}</p>
                  <p @click.stop="isEdit(index,true)" class="edit">编辑</p>
                </div>
            ...
            <el-button type="text" @click.stop="isEdit(false)" class="addInfo"><img src="/static/img/add.png" alt=""></el-button>
            <el-dialog title="收货地址" :visible.sync="dialogFormVisible">
                <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                  ...
                  <el-form-item>
                    <el-button type="primary" @click="submitForm('ruleForm',addressId)">确定</el-button>
                    <el-button type="danger" v-if="isDel" @click="del(addressId)">删除</el-button>
                    <el-button v-else @click="resetForm('ruleForm')">取消</el-button>
                  </el-form-item>
                </el-form>
              </el-dialog>
          </div>
        ...
        <div id="checkout-floatbar" class="group">
          <div class="checkout-buttons">
            <div class="sticky-wrap">
              <div class="inner">
                <button type="submit" class="checkout-submit" id="order-submit" @click.stop="submit_Order(ordergoods)" clstag="">
                  提交订单<b></b>
                </button>
              </div>
            </div>
...
<script>
  ...
  import {addAddress,addressDefault,delAddress} from './../../api/index';


  export default {
      name: "Shopping_order",
      computed: {
        ...mapState(['paygoods','addressInfo']),
          ...mapActions(['reqAddressInfo'])
      },
      mounted() {
          // 请求商品数据
      ...
      data(){
        ...
        return {
          ...
          addressId:0,//修改地址信息ID
          isDel:false,//是否地址信息修改页面
          ruleForm: {
            name: '',
            province: '',
            desc: '',
      ...
      methods:{
        ...
        getAllGoodsPrice() {
          ...
          //填写地址信息
        submitForm(formName,addressId) {
            let biao = this.ruleForm;
            this.$refs[formName].validate(async (valid) => {
                if (valid) {
                    const result = await addAddress(biao,addressId);
                    ...
                    await this.$store.dispatch('reqAddressInfo');
                } else {
        ...
        // 更改默认地址
        async selDefault(num) {
            const result = await addressDefault(num);
             await this.$store.dispatch('reqAddressInfo');
        },

        //修改还是填写地址信息
        async isEdit(num = -1, shu) {
            if (shu) {
                this.addressId = this.$store.state.addressInfo[num].address_id;
                this.isDel =true;
                let addressInfo = JSON.parse(this.$store.state.addressInfo[num].address_info);
                this.ruleForm.name = addressInfo.name;
                let arr = addressInfo.province.split("/"); //字符分割
                this.selectedOptions[0] = TextToCode[arr[0]].code;
                this.selectedOptions[1] = TextToCode[arr[0]][arr[1]].code;
                this.selectedOptions[2] = TextToCode[arr[0]][arr[1]][arr[2]].code;
                this.ruleForm.province = CodeToText[this.selectedOptions[0]] + '/' + CodeToText[this.selectedOptions[1]] + '/' + CodeToText[this.selectedOptions[2]]

                console.log(this.selectedOptions)
                console.log(this.ruleForm.province)
                this.ruleForm.desc = addressInfo.desc;
                this.ruleForm.phone = addressInfo.phone;
                this.ruleForm.email = addressInfo.email;
            } else {
                this.addressId=0;
                this.isDel = false;
                this.ruleForm.name = '';
        ...
        del(addressId) {
            this.$confirm('此操作将删除该地址信息, 是否继续?', '提示', {
                ...
                const result = await delAddress(addressId);
                this.$message({
                    type: 'success',
...
<style scoped lang="stylus" ref="stylesheet/stylus">
  ...
  * {
  ...
  .form-new .add_mi .edit{
    cursor:pointer;
    text-align:right;
    padding-right: 20px;
  }
  .form-new .add_mi .edit:hover{
    color:red;
  }
