...
// 3. 获取商品信息
  ...
  async reqAddressInfo({commit}) {
    ...
    commit(GET_ADDRESS, {addressInfo: result.message})
  },


  // 15. 获取订单管理数据
  async getOrderManageInfo({commit},params) {
    let result = await getOrderManage(params);
    console.log(result)
    commit(GET_Order_Manage, {orderManageInfo: result.message})
  }
