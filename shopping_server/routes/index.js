...
router.post('/api/save_pay_goods', (req, res) => {
    ...
    req.body.goods.forEach((value,index)=>{
        ...
        let pro_type = value.pro_type;
        let pro_stype = value.pro_stype;
        let goods_name = value.goods_name;
        let thumb_url = value.thumb_url;
        let color_type = value.color_type;
        let length_type = value.length_type;
        let price = value.price;
        let buy_count = value.buy_count;
        let is_pay = 0; // 0 未购买 1购买
        console.log(color_type)
        console.log(length_type)
        // 3. 清空数据
        ...
        //         res.json({err_code: 0, message: '服务器内部错误!'});
        //     } else {
                    let add_sql = "INSERT INTO shopping_order_cart(user_id, goods_id, pro_type, pro_stype, goods_name, thumb_url,color_type,length_type, price, buy_count, is_pay, AddTime) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, SYSDATE())";
                    let sql_params = [user_id,goods_id,pro_type, pro_stype, goods_name, thumb_url,color_type,length_type, price, buy_count, is_pay];


                    conn.query(add_sql, sql_params, (error, results, fields) => {
                        if (error) {
                            res.json({err_code: 0, message: '加入订单管理失败!'});
                        } else {
                            res.json({success_code: 200, message: '加入订单管理成功!'});
                        }
                    });
        //     }
        // });
...
router.post('/api/add_address_info', (req, res) => {
    ...
    // 2. 获取客户端传过来的地址信息
    let address_info = JSON.stringify(req.body.address_info);
    let address_id = req.body.address_id;

    // 3. 查询数据
    let sql_str = "SELECT * FROM shopping_address_list WHERE user_id = '" + user_id +"'";
    let num_str = "select count(*) from shopping_address_list where user_id= '" + user_id +"'";
    conn.query(num_str, (error, results, fields) =>{
        if (error) {
            res.json({err_code: 0, message: '服务器内部错误!'});
        } else {
            results = JSON.parse(JSON.stringify(results));
            ...
            if(num>3){
                res.json({err_code: 200, message: '填写地址不能超过3个!'});
                return;
            } else {
                conn.query(sql_str, (error, results, fields) => {
                    if (error) {
                        ...
                        if (results[0]) { // 3.1 商品已经存在
                            // 3. 插入地址数据
                            let sql_str = "SELECT * FROM shopping_address_list WHERE user_id = '" + user_id +"'and address_id = " + address_id;
                            conn.query(sql_str, (error, results, fields) => {
                                if (results[0]) {
                                    let add_sql = "update shopping_address_list SET address_info = '"+address_info+"' where user_id = '"+user_id + "' and address_id = "+address_id;
                                    // console.log(add_sql)
                                    conn.query(add_sql, (error, results, fields) => {
                                        if (error) {
                                            res.json({err_code: 0, message: '修改地址信息失败!'});
                                        } else {
                                            res.json({success_code: 200, message: '修改地址信息成功!'});
                                        }
                                    });
                                } else {
                                    let is_default = 0;//0 非默认地址
...
router.post('/api/address_default', (req, res) => {
    ...
    // 3. 查询数据
    let sql_str = "update shopping_address_list set is_default=0 where user_id = '"+user_id +"' and address_id <> " +address_id +
        ";update shopping_address_list set is_default=1 where user_id = '"+user_id +"' and address_id = " +address_id + ";" ;
    ...
    conn.query(sql_str, (error, results, fields) => {
        if (error) {
            ...
            if (results[0]) { // 3.1 地址已经存在
                res.json({success_code: 200, message: '修改默认地址成功!'});
            }
        }
    });
});
 ...
 * @type {Router}
 */
router.post('/api/edit_address_info', (req, res) => {
    // 1. 验证用户
    // console.log(req.body)
    let user_id = req.session.userId;
    // let user_id =1;
...
router.post('/api/del_address', (req, res) => {
    ...
    // 3. 查询数据
    let sql_str = "SELECT * FROM shopping_address_list WHERE address_id = '" + address_id + "' and user_id = '" + user_id + "' LIMIT 1";
    conn.query(sql_str, (error, results, fields) => {
        if (error) {
            res.json({err_code: 0, message: '服务器内部错误!'});
        } else {
            ...
            // console.log(results);
            if (results[0]) { // 3.1 商品已经存在
                let sql_str = "DELETE FROM shopping_address_list WHERE address_id = '" + address_id + "' and user_id = '" + user_id + "' LIMIT 1";
                conn.query(sql_str, (error, results, fields) => {
                    if (error) {
                        res.json({err_code: 0, message: '删除购物车商品失败!'});
                    } else {
 ...
 * 查询商品详细信息
 */
router.get('/api/order_manage_info', (req, res) => {
    //  获取客户端传过来的商品信息
    let user_id = req.session.userId;
    let is_pay = req.query.is_pay; //识别订单类别

    //  查询数据
    let sql_str = "SELECT * FROM shopping_order_cart WHERE user_id = '" + user_id + "' AND is_pay = " + is_pay;
    conn.query(sql_str, (error, results, fields) => {
        if (error) {
            res.json({err_code: 0, message: '请求数据失败'});
        } else {
